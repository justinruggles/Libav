                                 ...
                                 int filter_quant, int16_t *filter)
{
    ...
    predictors[0] = get_code(gb, 10);
    predictors[1] = get_code(gb, 10);
    predictors[2] = get_code(gb, size) << (10 - size);
    predictors[3] = get_code(gb, size) << (10 - size);
    if (filter_order > 4) {
        ...
        for (i = 4; i < filter_order; i++) {
            ...
            predictors[i] = get_code(gb, code_size) << (10 - size);
        }
    }
...
static int32_t filter_sample(DSPContext *dsp, const int16_t *residues,
                             const int16_t *filter, int order)
{
    int i, order1;
    int32_t sum = 0;
    ...
    if (order >= 16) {
        order1 = order & ~15;
        sum += dsp->scalarproduct_int16(residues, filter, order1);
        residues += order1;
        filter   += order1;
        order    -= order1;
    }
    if (order >= 8) {
        order1 = order & ~7;
        for (i = 0; i < order1; i += 8) {
            sum += residues[i + 7] * filter[i + 7] +
                   residues[i + 6] * filter[i + 6] +
                   residues[i + 5] * filter[i + 5] +
                   residues[i + 4] * filter[i + 4] +
                   residues[i + 3] * filter[i + 3] +
                   residues[i + 2] * filter[i + 2] +
                   residues[i + 1] * filter[i + 1] +
                   residues[i]     * filter[i    ];
        }
        residues += order1;
        filter   += order1;
    ...
    for (i = 0; i < order; i += 4) {
        sum += residues[i + 3] * filter[i + 3] +
               residues[i + 2] * filter[i + 2] +
               residues[i + 1] * filter[i + 1] +
               residues[i + 0] * filter[i + 0];
    }
                           ...
                           int subframe_size, int prev_subframe_size)
{
    ...
    av_fast_malloc(&s->residues, &s->residues_buf_size,
                   FFALIGN(subframe_size, 16) * sizeof(*s->residues));
    if (!s->residues)
    ...
    for (i = 0; i < subframe_size - filter_order; i++) {
        ...
        v += filter_sample(&s->dsp, &s->residues[i], filter, filter_order);
...
static int decorrelate(TAKDecContext *s, int c1, int c2, int length)
{
    ...
    case 7: {
        ...
        for (i = 0; i < filter_order; i++) {
            ...
            filter[i] = get_code(gb, code_size);
        }
        ...
        av_fast_malloc(&s->residues, &s->residues_buf_size,
                       FFALIGN(length, 16) * sizeof(*s->residues));
        if (!s->residues)
            return AVERROR(ENOMEM);
    ...
    if (s->nb_samples < 16) {
        for (chan = 0; chan < avctx->channels; chan++) {
            ...
            for (i = 0; i < s->nb_samples; i++)
                decoded[i] = get_code(gb, avctx->bits_per_coded_sample);
        }
    } else {
        if (s->ti.codec == TAK_CODEC_MONO_STEREO) {
            for (chan = 0; chan < avctx->channels; chan++)
